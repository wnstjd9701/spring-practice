...
<tr>
	<th>HIRE_DATE</th>
	<td><input type="text" name="hireDate" value="${emp.hireDate}" required></td>
</tr>
<tr>
	<th>JOB_ID</th>
	<td>
		...
		<c:forEach var="job" items="${jobList}">
			<option value="${job.jobId}" <c:if test="${emp.jobId == job.jobId}">selected</c:if>>
				${job.jobTitle}
			</option>
		</c:forEach>
		</select>
...
<tr>
	...
	<td>
		...
		<c:forEach var="manager" items="${managerList}">
			<option value="${manager.managerId}" <c:if test="${emp.managerId == manager.managerId}">selected</c:if>>
				${manager.firstName}
			</option>
		</c:forEach>
		</select>
